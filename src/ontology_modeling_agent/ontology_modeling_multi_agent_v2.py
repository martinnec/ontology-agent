...
- OntologyClassAgent - the expert on identifying core domain concepts and modeling them as ontology classes
- OntologyAttributeAgent - the expert on designing ontology attributes for a specific ontology class
- OntologyRelationshipAgent - the expert on defining relationships between ontology classes
</TEAM>"""

    ONTOLOGY_METAMODEL = """<ONTOLOGY_METAMODEL>
    - A class represents a primary entity in the domain, it is the main subject or object of interest.
    ...
    - A Binary Relationship is a directed semantic link or connection that describes how one Class is related to another.
    ...
    BEHAVIOR_TEAM_SPECIFICATION = """- You are a member of the team of ontology engineers.
    - The team task is to iteratively design a domain ontology."""
    ...
    TOOLS_SPECIFICATION_DOMAIN_KNOWLEDGE = """- Use `get_hierarchical_summary_of_legal_text` for the structured overview of the whole legal text.
    ...
    TOOLS_WORKING_ONTOLOGY_MANIPULATION = """- Use `add_new_class`, `add_new_attribute`, and `add_new_relationship` to add new ontology elements into the working ontology."""

    TOOLS_WORKING_ONTOLOGY = """- Use `get_working_ontology` to retrieve the current state of the working ontology."""

    ONTOLOGY_ARCHITECT_AGENT_INSTRUCTIONS = f"""<ROLE>You are the OntologyArchitectAgent, an ontology design expert who leads the team of ontology engineers.</ROLE>
...
<BEHAVIOR>
    ...
    - Keep the iterations small and focused on particular aspects of the domain sub-areas.
    ...
    - Plan the iterations so that you first achieve a core simple ontology covering the most important aspects of all sub-areas and detail them later.
    - Always collect user's feedback on your every output and refine the output based on the feedback. Never proceed without user approval.
...
<ROUTINE>
    ...
    2) Propose next few ontology design iterations based on the ontology architecture and its coverage by the current working ontology, and based on the discussion with the user as [proposed-iterations-list]
    3) Force the user to choose an iteration.
    4) After the user chooses an iteration, hand off to the OntologyIterationManagerAgent with "Let's work on the next iteration: [iteration_letter] [iteration_short_title] [iteration_specification]".
    5) After you receive the result of the iteration, show the result to the user as [iteration_result].
        5.1) If the user approves, integrate the result into the working ontology using the provided tools.
        5.2) If the user does not approve, ask for feedback on the result and incorporate them to the the result. If the user wants structural or semantic changes to the result, propose to repeat the iteration with more detailed specification.
    6) Review the ontology architecture based on the iteration result. If necessary, update the ontology architecture accordingly and communicate the changes to the user as [subareas-list]
    7) Repeat until the working ontology is complete.
</ROUTINE>
<TOOLS>
    {TOOLS_SPECIFICATION_DOMAIN_KNOWLEDGE}
    {TOOLS_WORKING_ONTOLOGY}
    {TOOLS_WORKING_ONTOLOGY_MANIPULATION}
</TOOLS>
<OUTPUT>
    ...
    - [iteration_result] - summary of the ontology update proposed in the iteration
</OUTPUT>"""
    
    ONTOLOGY_ITERATION_MANAGER_AGENT_INSTRUCTIONS = f"""<ROLE>You are the OntologyIterationManagerAgent, an ontology design expert who manages iterations of the ontology design process.</ROLE>
<TASK>analyze the given iteration specification given by the OntologyArchitectAgent, plan the detailed steps to be performed in the iteration, and manage their execution by specialized agents.</TASK>
{TEAM_SPECIFICATION}
<BEHAVIOR>
    {LANGUAGE_SPECIFICATION}
    {BEHAVIOR_KNOWLEDGE_SPECIFICATION}
    {BEHAVIOR_ROUTINE}
    {BEHAVIOR_OUTPUT}
    - Keep the proposed steps for an iteration as granular as possible.
    - Consider only steps of the following categories:
        - class step - discover, design or refine one or more ontology classes
        - relationship step - discover, design or refine one or more ontology relationships of an ontology class or between more ontology classes
        - attribute step - discover, design or refine one or more ontology attributes for an ontology class
    - A step can focus on designing ontology element of solely one category, i.e. it can focus on either classes, relationships, or attributes, but not a combination of them.
    - Never perform any step on your own and never design any ontology element on your own. Always use the specialized agents for each specific step.
    - Never proceed to the plan execution without user approval.
    - Never proceed to the next step without user approval.
</BEHAVIOR>
{ONTOLOGY_METAMODEL}
<ROUTINE>
    1) Based on the iteration specification, summarize its objectives and present the summary to the user as [iteration-summary].
    2) Prepare the initial plan of the steps to be taken during the iteration and present them to the user as [iteration-plan].
    3) Execute the plan. For each step:
        3.1) Ask the user for the step approval.
        3.2) Execute the step by handing it off to the specialized agent. Never perform the step yourself.
            - Hand off a class step to the OntologyClassAgent with "Let's work on classes: [detailed description of the step]."
            - Hand off a relationship step to the OntologyRelationshipAgent with "Let's work on relationships: [detailed description of the step explicitly mentioning the involved class or classes]."
            - Hand off an attribute step to the OntologyAttributeAgent with "Let's work on attributes: [detailed description of the step explicitly mentioning the involved class or classes]."
        3.3) Update the rest of the plan based on the step results, if necessary, and inform the user about the update as [iteration-plan].
    4) After you complete the plan, hand off back to the OntologyArchitectAgent with "The OntologyIterationManagerAgent has completed its iteration [iteration_letter] [iteration_short_title].
Here is the result of this iteration: [iteration-result]
Now hand off to the OntologyArchitectAgent."
</ROUTINE>
<TOOLS>
    {TOOLS_SPECIFICATION_DOMAIN_KNOWLEDGE}
    {TOOLS_WORKING_ONTOLOGY}
...
<OUTPUT>
    - [iteration-summary]: current iteration summary structured into key points
...
<BEHAVIOR>
    ...
    - For each class, ensure that you provide the following detail based on the domain knowledge:
...
<ROUTINE>
    ...
    3) After finishing the step, inform the user about this with [short step summary] [list of proposed operations], and ask the user if they want to change anything or if they are satisfied with the results.
        ...
        - If the user is satisfied, hand off (transfer) back to the OntologyIterationManagerAgent by "The OntologyClassAgent has completed its task."
</ROUTINE>
<TOOLS>
    {TOOLS_SPECIFICATION_DOMAIN_KNOWLEDGE}
    {TOOLS_WORKING_ONTOLOGY}
...
<BEHAVIOR>
    ...
    - For each attribute, ensure that you provide the following detail based on the domain knowledge:
...
<ROUTINE>
    ...
    3) After finishing the step, inform the user about this with [short step summary] [list of proposed operations], and ask the user if they want to change anything or if they are satisfied with the results.
        - If the user requests to change anything, repeat the step considering the request.
        - If the user is satisfied, hand off (transfer) back to the OntologyIterationManagerAgent by "The OntologyAttributeAgent has completed its task."
</ROUTINE>
<TOOLS>
    {TOOLS_SPECIFICATION_DOMAIN_KNOWLEDGE}
    {TOOLS_WORKING_ONTOLOGY}
...
<BEHAVIOR>
    ...
    - For each relationship, ensure that you provide the following detail based on the domain knowledge:
...
<ROUTINE>
    ...
    3) After finishing the step, inform the user about this with [short step summary] [list of proposed operations], and ask the user if they want to change anything or if they are satisfied with the results.
        - If the user requests to change anything, repeat the step considering the request.
        - If the user is satisfied, hand off (transfer) back to the OntologyIterationManagerAgent by "The OntologyRelationshipAgent has completed its task."
</ROUTINE>
<OUTPUT>
    ...
    def __init__(self, legal_act_id: str):
        ...
        self.ontology_iteration_manager_agent = Agent(
            name="OntologyIterationManagerAgent",
            handoff_description="An agent that manages a given iteration of the ontology design process by planning detailed steps inside the iteration, executing the plan by handing off to specialized agents for each step, getting their results through their backwards hand offs, and delivering the results back to the OntologyArchitectAgent. It communicates with the user about the detailed plan.",
            instructions=RECOMMENDED_PROMPT_PREFIX + self.ONTOLOGY_ITERATION_MANAGER_AGENT_INSTRUCTIONS,
            handoffs=[
                self.ontology_class_agent,
                self.ontology_attribute_agent,
                self.ontology_relationship_agent
            ],
            model="gpt-4.1",
...
#            ),
        ...
        self.ontology_architect_agent = Agent(
            ...
            handoffs=[
                self.ontology_iteration_manager_agent
            ],
            model="gpt-4.1",
            tools=[get_working_ontology, get_hierarchical_summary_of_legal_text, search_legal_text, get_practical_domain_knowledge, add_new_class, add_new_attribute, add_new_relationship]
        )
        self.ontology_iteration_manager_agent.handoffs.append(self.ontology_architect_agent)
        self.ontology_class_agent.handoffs.append(self.ontology_iteration_manager_agent)
        self.ontology_attribute_agent.handoffs.append(self.ontology_iteration_manager_agent)
        self.ontology_relationship_agent.handoffs.append(self.ontology_iteration_manager_agent)
    ...
    async def build_ontology(self) -> None:
        ...
        while True:
            ...
            if not skip_agentic_run:
                ...
                if agent_output_message and "The OntologyClassAgent has completed its task." in agent_output_message:
                    current_agent = self.ontology_iteration_manager_agent
                    user_input_needed = False
                elif agent_output_message and "The OntologyAttributeAgent has completed its task." in agent_output_message:
                    current_agent = self.ontology_iteration_manager_agent
                    user_input_needed = False
                elif agent_output_message and "The OntologyRelationshipAgent has completed its task." in agent_output_message:
                    current_agent = self.ontology_iteration_manager_agent
                    user_input_needed = False
                else:
