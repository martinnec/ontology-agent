...
class OntologyModelingAgent:
    ...
    AGENT_INSTRUCTIONS = """<ROLE>You are a Conceptual Designer Agent, a helpful ontology engineer.</ROLE>
<TASK>
...
4. For each class on the list:
    ...
    4.2. Discuss with the user about the gathered knowledge and refine it if necessary.
    4.3. Organize the gathered and refined knowledge about the class to the full list of class properties.
    4.4. Think hard about each property whether it is an intrinsic characteristic of the class itself that should be modeled as an attribute or it is a relationship to another class.
    4.6. Think hard about each attribute whether it is an intrinsic characteristic of the class itself or it is an intrinsic characteristic of some other class that should be identified and connected through a relationship.
    4.5. Think hard about each attribute whether it should not be further decomposed to more atomic attributes. If so, always decompose it before revealing to the user.
    4.7. If a relationship with a new class is identified, put this class onto the list of key classes in the subject area and get back to it later.
5. Validate the identified classes, attributes and relationships against the legal act and with the user to ensure accuracy and completeness.
6. Whenever during the process the user confirms an ontology element, you add it into the ontology and you inform the user shortly.
...
8. Ensure that each discovered class is discussed with the user.
9. Repeat the process until the ontology is complete (i.e. 100 percent coverage of the legal act and the knowledge gathered from the user), if necessary identify new key classes not included in the ontology yet.
</ROUTINE>
<OUTPUT>
...
The domain ontology comprises all discovered classes, their attributes and binary relationships.
    ...
    def __init__(self, legal_act_id: str):
        ...
        def search_legal_act(query: str, k: int) -> list[str]:
            ...
            Try to be as specific as possible when formulating your query, e.g.:
            ...
            If the query does not return anything relevant, relax the query and try again.

            Use the parameter k to determine the number of returned passages. If all returned passages are highly relevant, there may be other passages in the legal text that you did not receive so search again with increased k.
        ...
        @function_tool
        def add_new_class(prefLabel: str, definition: str, comment: str, references: list[str]) -> bool:
            """
            ...
            """
            return self._add_new_class_impl(prefLabel, definition, comment, references)
    ...
    def _add_new_class_impl(self, prefLabel: str, definition: str, comment: str, references: list[str]) -> bool:
        """
        ...
        return self.ontology_service.add_class(
            ...
            comment_cs=comment,
            comment_en="",
            parent_class_iri=None,  # No parent specified in simplified interface
